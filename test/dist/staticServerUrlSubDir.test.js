...
const runtimeDir = path.join(__dirname, 'runtime/staticServerUrlSubDir');
const distDir = path.join(runtimeDir, 'project/dist');

describe('dist staticServerUrl:subDir', () => {
  // 60s timeout
  ...
  test('dist: staticServerUrl:subDir', done => {
    const child = spawn('node', [path.join(__dirname, 'dist.js')], {
      env: Object.assign({}, process.env, {
        local: 'staticServerUrl:subDir',
        runtimeDir: 'runtime/staticServerUrlSubDir',
      }),
    });
    ...
    child.on('close', code => {
      ...
      expect(/[^\w/]\/sub\/dir\/absolute\/path\/image\.jpg/.test(htmlContent)).toBeTruthy();
      expect(/[^\w/]\/sub\/dir\/dist\/([0-9a-f]{32})\.ico/.test(htmlContent)).toBeTruthy();
      expect(/[^\w/]\/sub\/dir\/dist\/([0-9a-f]{32})\.png/.test(htmlContent)).toBeTruthy();
      expect(/[^\w/]\/sub\/dir\/dist\/([0-9a-f]{32})\.js/.test(htmlContent)).toBeTruthy();
      expect(/[^\w/]\/sub\/dir\/dist\/([0-9a-f]{32})\.css/.test(htmlContent)).toBeTruthy();
